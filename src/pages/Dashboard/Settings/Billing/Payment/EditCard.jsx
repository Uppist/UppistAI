...
export default function EditCard({ selectedCard, details, handleChange }) {
  console.log(selectedCard);

  const lastFourDigits = selectedCard?.cardNumber?.replace(/\s/g, "").slice(-4);
    ...
    <div className='flex flex-col gap-y-6 pl-15 pr-15'>
      ...
      <form action='' className='flex flex-col gap-y-6'>
        ...
        <div className='flex flex-col gap-y-1.5'>
          ...
          <div className='flex items-center gap-y-2 rounded-lg px-3 py-2 gap-x-2'>
            ...
            <span className='text-sm font-medium text-grey capitalize'>
              {selectedCard?.card_type} ···· {lastFourDigits}
            </span>
          </div>
        </div>
        {/*Cardholder name*/}
