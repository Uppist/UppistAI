...
export default function SearchDownload({
  ...
  const channels = ["All Channels", "WhatsApp", "Website", "Socials", "Email"];
  ...
  //search and filter contacts based on selected channel
  function handleChannelSelect(channel) {
    ...
    const filtered = contacts.filter((contact) => {
      const normalizedChannel = String(contact.channel || "")
        .toLowerCase()
        .trim();
      const normalizedSelection = channel.toLowerCase();

      if (normalizedSelection === "website" || normalizedSelection === "web") {
        return (
          normalizedChannel === "website" ||
          normalizedChannel === "web" ||
          normalizedChannel.includes("website") ||
          normalizedChannel.includes("web")
        );
      }

      if (normalizedSelection === "socials") {
        return (
          normalizedChannel.includes("social") ||
          normalizedChannel.includes("socials") ||
          normalizedChannel.includes("instagram") ||
          normalizedChannel.includes("facebook")
        );
      }

      return normalizedChannel === normalizedSelection;
    });
    ...
    <div className='flex items-center justify-between'>
      ...
      <div className='flex relative items-center gap-x-5'>
        ...
        <div className='relative'>
          <div
            className='flex items-center gap-x-2 border border-light-grey py-1.5 px-3 rounded-lg shadow-sm cursor-pointer'
            onClick={() => {
            ...
            <div className='flex absolute flex-col bg-white rounded-lg shadow-2xl p-4 gap-y-8 w-50'>
              {channels.map((channel) => (
                <button
                  key={channel}
                  type='button'
                  className='text-sm flex items-baseline font-medium text-light-black cursor-pointer'
        ...
        <div>
          <div
            className='flex relative items-center gap-x-2 border border-light-grey py-1.5 px-3 rounded-lg shadow-sm'
        ...
        <CSVLink
          ...
          className='flex items-center gap-x-2 rounded-lg bg-bg border-none text-white text-sm font-semibold py-2.5 px-3 cursor-pointer hover:opacity-50 no-underline'
        >
          <svg
            width='20'
            height='20'
